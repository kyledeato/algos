 ...
 */
 var floodFill = function(image, sr, sc, newColor) {
    // checks if it is out of bounds and selected color is same as new color
    if (image == null || image.length < 1 || image[sr][sc] == newColor){
        return image
    }
    
    
    // finds the initial color
    const initColor = image[sr][sc]
    
    //recursive call
    const fill = (image, sr, sc, newColor, initColor) => {
        // if sr or sc are out of bounds and the sr and sc are different from the initial color i.e the 0's != 1
